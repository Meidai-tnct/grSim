...
#define ADD_VALUE(parent,type,name,defaultvalue,namestring) \
    v_##name = shared_ptr<VarTypes::Var##type>(new VarTypes::Var##type(namestring,defaultvalue)); \
...
ConfigWidget::ConfigWidget() {
	...
	world = VarXML::read(world, (QDir::homePath() + QString("/.grsim.xml")).toLocal8Bit().constData());
	...
	for (int i = 0; i < list.size(); ++i) {
		...
		ADD_TO_ENUM(BlueTeam, str.toLocal8Bit().constData())
			ADD_TO_ENUM(YellowTeam, str.toLocal8Bit().constData())
	}
...
ConfigWidget::~ConfigWidget() {
	VarXML::write(world, (QDir::homePath() + QString("/.grsim.xml")).toLocal8Bit().constData());
}
