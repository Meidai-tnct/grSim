...
QImage* createBlob(char yb, int i, QImage** res) {
	*res = new QImage(QString("../resources/textures/%1%2").arg(yb).arg(i) + QString(".png"));
	return *res;
}
...
void SSLWorld::glinit() {
	g->loadTexture(new QImage("../resources/textures/grass.png"));
	...
	for (int i = 0; i < 6; i++) {
		QString a = (QString("../resources/textures/sky/neg_%1").arg(i % 3 == 0 ? 'x' : i % 3 == 1 ? 'y' : 'z') + QString(".png"));
		g->loadTexture(new QImage(QString("../resources/textures/sky/neg_%1").arg(i % 3 == 0 ? 'x' : i % 3 == 1 ? 'y' : 'z') + QString(".png")));
		g->loadTexture(new QImage(QString("../resources/textures/sky/pos_%1").arg(i % 3 == 0 ? 'x' : i % 3 == 1 ? 'y' : 'z') + QString(".png")));
	}

	// The wheel texture
	g->loadTexture(new QImage("../resources/textures/wheel.png"));
...
void SSLWorld::recvActions() {
	...
	while (commandSocket->hasPendingDatagrams()) {
		...
		if (size > 0) {
			logStatus(in_buffer, QColor("black"));
			for (int i = 0; i < 6; i++) {/////////////////////////ここのスコープがKIKS
				int McID = GetStrToInt(in_buffer);
				dReal vx = 1.0*GetStrToInt(in_buffer) / 2000.;
				dReal vy = 1.0*GetStrToInt(in_buffer) / 2000.;
				dReal vw = 1.0*GetStrToInt(in_buffer) / 16.;
				robots[McID]->setSpeed(vx, vy, vw);

				dReal kickx = 0, kickz = 0;
				bool kick = false;
				kick = GetStrToInt(in_buffer);
				GetStrToInt(in_buffer);
				int rolling = 0; rolling = GetStrToInt(in_buffer);
				robots[McID]->kicker->setRoller(rolling);
				if (GetStrToInt(in_buffer) && kick) {
					kickz = GetStrToInt(in_buffer);
					kickx = kickz;
					robots[McID]->kicker->kick(kickx, kickz);
				} else if (kick) {
					kickx = GetStrToInt(in_buffer);
					robots[McID]->kicker->kick(kickx, 0);
				} else {
					GetStrToInt(in_buffer);
				}
			}
...
int SSLWorld::GetStrToInt(const char str[]) {
	int		j;
	int		nowConut = m_nStrPos;	// これには意味がある(はず...)
	char	tmp[1 + 10 + 1];			// 符号＋32ビット整数の桁数＋文字列終端文字　の分

	//最大バイト数をjが超えないようにという条件文
	for (j = 0; j < (sizeof(tmp) / sizeof(tmp[0])) - 1 && str[j + nowConut] != ' '; j++) {
		tmp[j] = str[j + nowConut];
	}
	tmp[j] = '\0';
	m_nStrPos += (j + 1);	// 取り出した文字数＋スペース分
